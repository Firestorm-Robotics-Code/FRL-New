...
public:
    ...
    void SetPercent(double perc) {
        ...
        if (perc > hardMaxOutput) {
            Motor_printWarning("A TalonSRXMotor with CANId " + std::to_string(CANId) + " went over hard \npercent output of " + std::to_string(hardMaxOutput) + "! Aborting now.");
            assert(perc > hardMaxOutput);
        }
        if (perc < hardMinOutput) {
            Motor_printWarning("A TalonSRXMotor with CANId " + std::to_string(CANId) + " went under hard \npercent output of " + std::to_string(hardMaxOutput) + "! Aborting now.");
